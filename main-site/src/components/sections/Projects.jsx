...
const Projects = () => {
  ...
  const projects = [
    ...
    },
    {
      ...
      links: {
        ...
        github: 'https://github.com/Sandith02/glamour_tech_villa_management_suite'
      }
    },
    {
      id: 'car-rental',
  ...
  return (
    <section id="projects" ref={sectionRef} className="py-16 sm:py-32 px-4 sm:px-6 bg-white">
      <div className="max-w-7xl mx-auto">
        ...
        <div className={`text-center mb-12 sm:mb-20 transition-all duration-1000 ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'}`}>
          <div className="inline-block px-3 sm:px-4 py-2 bg-gray-100 rounded-full text-xs sm:text-sm font-medium text-gray-600 mb-4 sm:mb-6">
            Projects
          </div>
          <h2 className="text-4xl sm:text-6xl md:text-7xl font-light text-gray-900 mb-6 sm:mb-8 tracking-tight leading-tight">
            Product<br />
        ...
        <div className="space-y-8 sm:space-y-16">
          {projects.map((project, index) => (
            ...
            >
              <div className="bg-white rounded-xl sm:rounded-2xl p-4 sm:p-8 md:p-12 shadow-sm hover:shadow-lg transition-all duration-300">
                
                {/* Project Header */}
                <div className="flex flex-col mb-6 sm:mb-8">
                  <div className="mb-4 sm:mb-6">
                    <div className="flex flex-col sm:flex-row sm:items-center gap-2 sm:gap-4 mb-3">
                      <h3 className="text-xl sm:text-2xl md:text-3xl font-bold text-gray-900 leading-tight">{project.title}</h3>
                      <span className="px-2 sm:px-3 py-1 bg-green-100 text-green-800 text-xs sm:text-sm font-medium rounded-full self-start">
                        {project.status}
                      </span>
                    </div>
                    <p className="text-base sm:text-lg text-[#394a43] font-medium mb-2">{project.subtitle}</p>
                    <p className="text-sm sm:text-base text-gray-600">{project.period}</p>
                  </div>
                  
                  {/* Metrics - Mobile Optimized */}
                  <div className="grid grid-cols-3 gap-3 sm:gap-6 mb-4 sm:mb-0">
                    {Object.entries(project.metrics).map(([key, value]) => (
                      <div key={key} className="text-center">
                        <div className="text-lg sm:text-xl md:text-2xl font-bold text-[#394a43] leading-tight">{value}</div>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 sm:gap-8">
                  ...
                  <div>
                    <h4 className="text-lg sm:text-xl font-semibold text-gray-900 mb-3 sm:mb-4">Business Impact</h4>
                    <ul className="space-y-2 sm:space-y-3">
                      {project.businessImpact.map((impact, i) => (
                        <li key={i} className="flex items-start">
                          <svg className="w-4 h-4 sm:w-5 sm:h-5 text-[#394a43] mr-2 sm:mr-3 mt-1 sm:mt-0.5 flex-shrink-0" fill="currentColor" viewBox="0 0 20 20">
                            <path fillRule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm3.707-9.293a1 1 0 00-1.414-1.414L9 10.586 7.707 9.293a1 1 0 00-1.414 1.414l2 2a1 1 0 001.414 0l4-4z" clipRule="evenodd" />
                          </svg>
                          <span className="text-sm sm:text-base text-gray-700 leading-relaxed">{impact}</span>
                        </li>
                      ))}
                    </ul>
                  </div>
                  ...
                  <div>
                    <h4 className="text-lg sm:text-xl font-semibold text-gray-900 mb-3 sm:mb-4">Key Skills Applied</h4>
                    <div className="mb-4 sm:mb-6">
                      <div className="flex flex-wrap gap-1.5 sm:gap-2 mb-4">
                        {project.pmSkills.map((skill) => (
                          <span key={skill} className="px-2 sm:px-3 py-1 bg-[#394a43] bg-opacity-10 text-[#394a43] text-xs sm:text-sm font-medium rounded-full">
                            {skill}
                          </span>
                        ))}
                      </div>
                    ...
                    <h5 className="text-base sm:text-lg font-semibold text-gray-900 mb-2 sm:mb-3">Technology Stack</h5>
                    <div className="flex flex-wrap gap-1.5 sm:gap-2">
                      {project.tech.map((tech) => (
                        <span key={tech} className="px-2 sm:px-3 py-1 bg-gray-100 text-gray-700 text-xs sm:text-sm font-medium rounded-full">
                          {tech}
                        </span>
                      ))}
                    </div>
                    ...
                    <div className="flex flex-col sm:flex-row gap-3 sm:gap-4">
                      {project.links.live && (
                        <a 
                          ...
                          className="inline-flex items-center justify-center px-4 py-2.5 sm:py-2 bg-[#394a43] text-white font-medium rounded-lg hover:bg-opacity-90 transition-all duration-300 text-sm sm:text-base"
                        >
                      ...
                      {project.links.github && (
                        <a 
                          ...
                          target="_blank" 
                          rel="noopener noreferrer"
                          className="inline-flex items-center justify-center px-4 py-2.5 sm:py-2 border border-gray-300 text-gray-700 font-medium rounded-lg hover:bg-gray-50 transition-all duration-300 text-sm sm:text-base"
                        >
        ...
        {/* Bottom CTA */}
        <div className={`text-center mt-12 sm:mt-20 transition-all duration-1000 delay-1000 ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'}`}>
          ...
          <button
            ...
            className="inline-flex items-center px-6 sm:px-8 py-3 sm:py-4 bg-[#394a43] text-white font-semibold rounded-lg hover:bg-opacity-90 transition-all duration-300 shadow-lg hover:shadow-xl group text-sm sm:text-base"
          >
            Let's Talk Strategy
